...
autoplot(na.omit(ts_data$total_users_holding), main='Robinhood User Growth', ylab='# of Unique Users Owning a Top 50 Stock')
...
# Graph shows how VIX and users holding ln return moves similarly.
...
# Prewhiten to help remove white noise before correlating with avg users holding ln returns
lm_resid_sq_fit = auto.arima(lm_resid_sq)
summary(lm_resid_sq_fit)
acf(lm_resid_sq_fit$residuals)
Box.test(lm_resid_sq_fit$residuals, lag=10, type="Ljung")
prewhite = prewhiten(lm_resid_sq, robinhood_xts2020$avg_users_holding_ln_ret, lm_resid_sq_fit)
autoplot(prewhite)
# Lag -1 is shown to be the best correlation
...
lag2.plot(lm_resid_sq, robinhood_xts2020$avg_users_holding_ln_ret, 3)
...
final_plot = ggplot(combined_fit_xts, aes(x = vix_cur_Close, y = vol_ts)) +
  ...
  scale_color_gradient(low="blue", high="red") + labs(col='Lag 1 of users holding') + ggtitle('LM Improvement w/ Robinhood Users') + xlab('SPX volatility') + ylab('VIX')
final_plot 
...
ggplot(combined_fit_xts, aes(x = Index, y = vix_cur_Close)) +
  ...
  theme(legend.position = c(.98, .98),
        ...
        legend.box.just = "left",
        legend.title = element_text(size=8)) + ggtitle('ARIMA Model Comparison') + xlab(NULL) + ylab('VIX')
